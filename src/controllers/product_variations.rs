...
pub struct ProductVariationCreate {
    ...
    /// List of attributes.
    pub attributes: Vec<DefaultAttributeDTO>,
...
mod tests {
    ...
    async fn test_create_product_variation() {
        let client = ApiClient::from_env().unwrap();
        let mut variation_to_create = std::collections::HashMap::new();
        variation_to_create.insert("sku", "testsku");
